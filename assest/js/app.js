...
function initTrivia(){
  ...
  options.forEach(opt=>{
    opt.addEventListener('click', ()=>{
      options.forEach(o=>o.style.background='#ffd6e8');
      opt.style.background='#ff85a2';
    });
  });
...
document.addEventListener('DOMContentLoaded', ()=>{
  if(document.querySelector('.film-item')) initFilmRoll();
  if(document.querySelector('.photo-holder')) initPhotoDeck();
  if(document.getElementById('pbCanvas')) initPhotoBooth();
  if(document.querySelector('.video-box')) initVideoPage();
  if(document.querySelector('.trivia-page')) initTrivia();
});
